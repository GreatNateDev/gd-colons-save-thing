...
function appendToTable(lvlList) {
    ...
    if (lvlList) {
        ...
        searchTable = lvlList.filter(x => !(x.type == "Official" && x.id > 1 && x.name == "Stereo Madness")) // remove broken spinoff levels :(
    }
...
function appendLevels(lvlList, res=[]) {
        ...
        lvlList.forEach(x => {
           ...
           let coinColor = official ? "" : x.verifiedCoins ? "silver" : "brown"
           let levelRating = official ? "Official" : x.epic ? "Epic" : x.featured ? "Featured" : x.stars ? (!x.name ? "Rated" : "Starred") : "Unavailable"
           ...
           let ratings = {
            ...
            let ratingStuff = ""
            Object.keys(ratings).forEach(z => { if (ratings[z]) ratingStuff += `<img height="40px" style="margin: 5 5 5 5" src="assets/ratings/${z}.png">` })
            ...
            <td>
                <p style="color: lime${official || !x.name ? hideStuff : ""}">     <img src="assets/download.png">${commafy(x.downloads || "?")}</p>
                <p style="color: ${isCringe ? "salmon" : "yellow"}${official || !x.name ? hideStuff : ""}">   <img src="assets/${isCringe ? "dis" : ""}like.png">${commafy(x.likes || "?")}</p>
                <p style="color: white${official || !x.name ? hideStuff : ""}">    <img src="assets/time.png">${x.length || "Unknown"}</p>
                ${x.secretCoinsToUnlock ? `<p style="color: yellow"><img src="assets/coin.png">${x.secretCoinsToUnlock} required</p>` : ""}
            </td>
            <td>
                <p style="color: ${x.percentage == 100 ? "yellow" : "white"}${starCount ? "" : hideStuff}">          <img src="assets/star.png">${starCount ? `${x.percentage == "100" ? starCount : 0}/${starCount}` : 0}</p>
